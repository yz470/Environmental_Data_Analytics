...
EPAair <- unite(EPAair, Date, Year, Month, Day)
EPAair$Date <- as.Date(EPAair$Date, format = "%Y_%m_%d")
...
Plot_surface_T <- ggplot(subset(PeterPaul.chem.nutrients, depth = 0) , aes(x = daynum, y = temperature_C, color = year4)) +
  geom_point() +
  facet_wrap(vars(lakename), nrow = 2)
print(Plot_surface_T)

#2. 
# Plot temperature by date. Color your points by depth.
# Change the size of your point to 0.5
Plot_depth_T <- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth)) +
  geom_point(size = 0.5) 
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #specify linear model
  geom_abline(aes(slope = 16, intercept = 0),lty=2) #lty-line type 
print(NvsP)
...
Nutrientplot <- 
  ...
  geom_bar(aes(x = lakename, y = concentration, fill = as.factor(nutrient)), # why did we use fill?
           position = "dodge", stat = "summary", fun.y = "mean")             # what's happening here? "position = dodge" separates
print(Nutrientplot)
...
EPAair_summary <- EPAair %>%
  ...
  summarize(sd = sd(PM2.5, na.rm = TRUE), mean = mean(PM2.5, na.rm = TRUE))


ggplot(EPAair_summary, aes(y = mean, x = as.factor(Year), fill = Site.Name )) +
