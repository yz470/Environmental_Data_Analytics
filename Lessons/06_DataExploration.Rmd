...
```{r}
# remove the "/" in the date format 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# create a function that adds the year to the beginning of the date
...
> Answer: When analyzing a survey, "NA" has some valuable information.

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # delete the entire row that contains NA
dim(USGS.flow.data)
...
# color in aesthetic, plot the mean separately for approval and pending 
ggplot(USGS.flow.data.complete) +
